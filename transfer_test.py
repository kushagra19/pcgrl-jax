...
from conf.config import Config, TrainConfig, TransferConfig
from envs.pcgrl_env import (gen_dummy_queued_state, gen_dummy_queued_state_old,
                            OldQueuedState)
from purejaxrl.experimental.s5.wrappers import LogWrapper
from utils import (get_ckpt_dir, get_exp_dir, init_network, gymnax_pcgrl_make, gymnax_pcgrl_make_transfer, init_network_transfer, init_config)
    ...
    #                             fill_value=jnp.nan, dtype=jnp.float32)
        ...
        def render_episodes(network_params):
            _, (states, rewards, dones, infos, frames) = jax.lax.scan(
                step_env_render, (rng_r, obsv_r, env_state_r, network_params,gameNum),
...
def init_checkpointer(config: Config) -> Tuple[Any, dict]:
    ...
    env, env_params = gymnax_pcgrl_make_transfer(config.env_name, config=config)
    # env = FlattenObservationWrapper(env)
...
@hydra.main(version_base=None, config_path='./conf', config_name='transfer_pcgrl')
def main(config: TransferConfig):
